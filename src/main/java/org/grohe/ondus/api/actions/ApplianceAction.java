...
public class ApplianceAction extends AbstractAction {
    ...
    public List<Appliance> getAppliances(Location inLocation, Room inRoom) throws IOException {
        ApiResponse<Appliance[]> locationsResponse = getApiClient()
                .get(String.format(APPLIANCES_URL_TEMPLATE, inLocation.getId(), inRoom.getId()), Appliance[].class);
    ...
    public Optional<Appliance> getAppliance(Location inLocation, Room inRoom, String applianceId) throws IOException {
        ApiResponse<Appliance> applianceApiResponse = getApiClient()
                .get(String.format(APPLIANCE_URL_TEMPLATE, inLocation.getId(), inRoom.getId(), applianceId), Appliance.class);
        if (applianceApiResponse.getStatusCode() != 200) {
            return Optional.empty();
        }
        return applianceApiResponse.getContent();
    }
}
